...
void Net_CPU::train(host_vector< host_vector<float> >& Xtrain, host_vector<int>& Ytrain)
{
	dim3 threadsPerBlock(TILE_WIDTH,TILE_WIDTH);
	cout << "Net_CPU Train Start" << endl << fflush;
...
//// CxHxW
	...
	for(int epoch=1 ; epoch <= 100 ; epoch++){
		...
		while(minibatch_index < NUM_TRAIN/MINIBATCH)
		{
			...
			conv1.X_c = Xtrain[minibatch_index];

			conv1.forward_CPU();

			pool1.forward_CPU(conv1.Output_c);

			pool1.Output = pool1.Output_c;
...
void Net_CPU::test(host_vector< host_vector<float> >& Xtest, host_vector<int>& Ytest)
{
	dim3 threadsPerBlock(TILE_WIDTH,TILE_WIDTH);
	cout << "Net_CPU Test Start" << endl << fflush;
	int minibatch_index = 0;
