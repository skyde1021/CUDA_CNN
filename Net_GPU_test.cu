...
void Net_GPU_test::train(host_vector< host_vector<float> >& Xtrain, host_vector<int>& Ytrain)
{
	dim3 threadsPerBlock(TILE_WIDTH,TILE_WIDTH);
	cout << "Net_GPU_test Train Start" << endl << fflush;
	...
	for(int epoch=1 ; epoch <= 3 ; epoch++){
		...
		//// fc backward
			...
			if((minibatch_index*MINIBATCH) == 60000)
			{
				...
				printf("[Epoch %d] minibatch %d (%.7lf images/sec) acc %.3f elapsed time %.3f\n",
						epoch, minibatch_index , minibatch_index*MINIBATCH/elapsed, acc*100, elapsed);fflush(stdin);
			}
		}
	}
}

void Net_GPU_test::test(host_vector< host_vector<float> >& Xtest, host_vector<int>& Ytest)
{
	dim3 threadsPerBlock(TILE_WIDTH,TILE_WIDTH);
	cout << "Net_GPU_test Test Start" << endl << fflush;
	int minibatch_index = 0;
