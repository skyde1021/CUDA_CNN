...
void Net_GPU_Gemm::train(host_vector< host_vector<float> >& Xtrain, host_vector<int>& Ytrain)
{
	dim3 threadsPerBlock(TILE_WIDTH,TILE_WIDTH);
	cout << "Net_GPU_Gemm Train Start" << endl << fflush;
...
void Net_GPU_Gemm::test(host_vector< host_vector<float> >& Xtest, host_vector<int>& Ytest)
{
	dim3 threadsPerBlock(TILE_WIDTH,TILE_WIDTH);
	cout << "Net_GPU_Gemm Test Start" << endl << fflush;
	int minibatch_index = 0;
